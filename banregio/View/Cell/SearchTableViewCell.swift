...
class SearchTableViewCell: UITableViewCell {
    ...
    func configureCell(with searchTerm:String, cellText:String)
    {
        ...
        do {
            ...
            let highlightColour = UIColor(red: 124/255.0, green: 215/255.0, blue: 204/255.0, alpha: 0.5)
            
            regEx.enumerateMatches(in: cellText, options: .withTransparentBounds, range: range, using: { (result, flags, stop) in
                
                if result?.range != nil
                {
                    displayString.setAttributes([NSAttributedStringKey.backgroundColor:highlightColour], range: result!.range)
                }
